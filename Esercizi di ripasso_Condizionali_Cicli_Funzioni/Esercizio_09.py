...
Tale funzione deve utilizzare un ciclo per iterare la lista e, per ogni dimensione non compressa, deve calcolare 
la dimensione compressa di tale file espressa come float (ovvero deve calcolare l' 80% della dimensione non compressa), 
calcolare il numero di blocchi (arrotondato al numero intero più vicino) da 512 byte necessari per la memorizzazione, 
al fine di determinare se il file compresso può essere salvato nello spazio rimanente nel supporto di 
memorizzazione o meno.
 
In caso affermativo, il programma memorizza il file. In tal caso, la funzione deve stampare la dimensione originale 
del file, la dimensione compressa, i blocchi utilizzati per memorizzare il file in questione e i blocchi disponibili 
rimasti sul supporto di memorizzazione. 
...
def memorizza_file(files: list[int]) -> None:
    ...
    for file in files:
        dimensione_compressa = file * 0.8
        blocchi = round(dimensione_compressa / dimensione_blocco)
    
        if blocchi <= spazio_totale_blocchi:
            spazio_totale_blocchi -= blocchi
            print(f"File di {file} byte compresso in {dimensione_compressa} byte e memorizzato. Blocchi usati: {blocchi}. Blocchi rimanenti: {spazio_totale_blocchi}")
