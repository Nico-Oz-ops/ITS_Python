...
# Alternativa 2 - più pythonica
def voto_minimo_per_studente(studenti: list[dict[str, dict[str, list[int]]]]) -> dict[str, dict[str, int]]:
    risultato = {}

    for studente in studenti:
        voto_minimo = {materia: min(voti) for materia, voti in studente["materie"].items()}
        risultato[studente["nome"]] = voto_minimo
...
def voto_minimo_per_studente(studenti: list[dict[str, dict[str, list[int]]]]) -> dict[str, dict[str, int]]:
    ...
    for studente in studenti:
        ...
        for materia in studente["materie"]:
            voto_minimo = studente["materie"][materia][0]
            for voto in studente ["materie"][materia]:
                if voto < voto_minimo:
                    voto_minimo = voto
            minimi[materia] = voto_minimo
        risultato[studente["nome"]] = minimi
